...
parser.add_argument(
    "-c",
    "--path_to_demultiplex_stats",
    required=True,
    help="Path to demultiplex stats CSV",
)
parser.add_argument(
    "-r", "--path_to_run_info", required=True, help="Path to run info XML"
)
parser.add_argument(
    "-x", "--path_to_sample_sheet", required=True, help="Path to sample sheet CSV"
)
...
parser.add_argument(
    "-e",
    "--send_upload_receipts_to",
    required=True,
    help="Space-separated emails for upload receipts",
)

args = parser.parse_args()


def upload_demultiplex_stats(
    path_to_demultiplex_stats,
    path_to_run_info,
    path_to_sample_sheet,
    db_name,
    schema_name,
    db_user,
    db_password,
    ...
    dt = datetime.strptime(unformatted_seq_date, "%Y-%m-%dT%H:%M:%SZ").replace(
        tzinfo=timezone.utc
    )
    ...
    dmux_stats["sequencing_tube_tag"] = dmux_stats["Lane"].apply(
        lambda x: pool_lanes[x]
    )
