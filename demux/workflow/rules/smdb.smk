...
rule smdb_upload:
    ...
    shell:
        "./scripts/smdb-upload/smdb_upload.py --path_to_demultiplex_stats {input.demux_stats} --path_to_run_info {input.run_info} --path_to_sample_sheet {input.sample_sheet} {params.extra} --db_password {params.db_password:q} --send_upload_receipts_to {params.mail:q} > {output.log} 2>&1"
